...
public class ElectionUtil {
	...
	public static void startLeaderElection(){
		try{
			initElectionAttributes();
		if(isHappening== true&& ServerState.state.equals("Candidate")){
			broadCastVote(SingletonEgde.getInstance().getAllEdgesmap());
			int time = (int) (System.currentTimeMillis()+responseWaitTime);
			//boolean hasWon=false;
			while(System.currentTimeMillis()<=time&&totalVoteReceivedCount!=(SingletonEgde.getInstance().getAllEdgesmap().size()+1)&&notRespondedEdges.size()!=0){	
			try {
				Thread.sleep(20);
			...
			boolean flag=false;
			int majoritySize = (SingletonEgde.getInstance().getAllEdgesmap().size()/2)+1;
			int count=0;
			while(totalVoteReceivedCount<majoritySize&&count<2){	
			try {
					Thread.sleep(20);
					count+=1;
					if(isHappening== true&& ServerState.state==ServerLeaderElectionState.CANDIDATE){
						broadCastVote(notRespondedEdges);
					}
					else {
						flag=true;
			...
			if(flag==false){
				if(voteYes>=majoritySize&&isHappening==true&&ServerState.state==ServerLeaderElectionState.CANDIDATE){
	...
	public class BroadCastVote implements Runnable {
		...
		public void run() {
			// TODO Auto-generated method stub
			try{
			if (ei.isActive() && ei.getChannel() != null && isHappening== true&& ServerState.state==ServerLeaderElectionState.CANDIDATE) {
	...
	private static void initElectionAttributes()
	{
		isHappening= true;
		voteYes+=1;
		...
		notRespondedEdges = new ConcurrentHashMap<Integer,EdgeInfo>(SingletonEgde.getInstance().getAllEdgesmap());
		ServerState.state=ServerLeaderElectionState.CANDIDATE;
	}
	
	private static void updateElectionAttributes() {
		// TODO Auto-generated method stub
		isHappening=false;
