...
public class HasVoteRequestState extends WorkMessageState{
	...
	public void execute() {
		...
		try{
		if(SingletonEgde.getInstance().getAllEdgesmap().containsKey(msg.getHeader().getNodeId())){
			ElectionUtil.state=workHandler.getState();
			ElectionUtil.createVoteResponse(msg.getVoteRequest(),SingletonEgde.getInstance().getAllEdgesmap().get(msg.getHeader().getNodeId()));
		}
		}
		catch(Exception e)
		{
