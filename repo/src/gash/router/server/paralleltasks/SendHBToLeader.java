	...
	public void run() {
		try {
			if("Follower".equals(ServerState.state) && LeaderDetails.isLeaderDetailsSet()){
				EdgeInfo ei = SingletonEgde.getInstance().getAllEdgesmap().get(LeaderDetails.id);
				if (ei.isActive() && ei.getChannel() != null) {
					if(!ei.getChannel().isActive()){
						...
						WorkMessage wm = WorkMessageUtils.getHeartBeatAck(SingletonEgde.getInstance().getAllEdgesmap().get(LeaderDetails.id));
