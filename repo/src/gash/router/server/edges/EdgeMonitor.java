...
public class EdgeMonitor implements Runnable {
	...
	public static final String dnsHost1 = "169.254.203.42";
	public static final int dnsPort1 = 4569;
	public static final String dnsHost2 = "169.254.203.43";
	public static final int dnsPort2 = 4569;
	int electionTimeOut=0;
	public EdgeMonitor(){
		electionTimeOut=getRandomNum((((state.getConf().getNodeId())%4)+1)*600,(((state.getConf().getNodeId())%4)+1)*500);
	}
	...
	public void run() {
		...
		while (true) {			
			if(ServerState.state.equals("Leader")){
				sendHBToDNS();
				for (EdgeInfo ei : SingletonEgde.getInstance().getAllEdgesmap().values()) {
					WorkMessage wm = WorkMessageUtils.createHB(ei);
					broadCastWorkMessage(wm);
				}	
				...
				try {
					...
					if(LeaderDetails.receivedHBFromLeader)
						...
						if(!ElectionUtil.isHappening && SingletonEgde.getInstance().getActiveEdges().size()!=0){
							ElectionUtil.state=state;
							ServerState.state=ServerLeaderElectionState.CANDIDATE;
		...
		for(EdgeInfo ei: SingletonEgde.getInstance().getAllEdgesmap().values()){
			if (ei.isActive() && ei.getChannel() != null) {
				if(!ei.getChannel().isActive()){
					ei.setActive(false);
					ei.setChannel(null);
					SingletonEgde.getInstance().getAllEdgesmap().put(ei.getRef(), ei);
				}
				else {
	...
	public static WorkMessage dnsHB1=null;
	public static WorkMessage dnsHB2=null;
	
	public static void sendHBToDNS(){
		if(dnsHB1==null){
			EdgeInfo dnsEI1= new EdgeInfo(11,dnsHost1, dnsPort1);
			dnsHB1 = WorkMessageUtils.createHB(dnsEI1);
		}
		if(dnsHB2==null){
			EdgeInfo dnsEI2= new EdgeInfo(11,dnsHost1, dnsPort1);
			dnsHB2= WorkMessageUtils.createHB(dnsEI2);
		}		
		broadCastWorkMessage(dnsHB1);
		broadCastWorkMessage(dnsHB2);
	}
